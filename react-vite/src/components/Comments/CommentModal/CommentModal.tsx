...
interface PostComment {
	...
	created_at: string;
	updated_at: string;
	// FIXED: Include like data from API
	likes?: number;
	isLiked?: boolean;
	commenter?: {
		id: number;
...
const CommentModal: React.FC<CommentModalProps> = ({
	...
	useEffect(() => {
		if (commentsData && commentsData.length > 0) {
			...
			setCommentLikeStates(newLikeStates);

			console.log("Final initialized like states:", newLikeStates);
		}
	}, [commentsData]);
	...
	const flattenCommentStructure = useCallback(
		(comments: Comment[]): PostComment[] => {
			...
			const processComment = (comment: Comment) => {
				// Add the main comment with like data
	...
	const organizeComments = useCallback(
		(flatComments: PostComment[]): Comment[] => {
			...
			flatComments.forEach((pc) => {
				...
				// FIXED: Get like state - prioritize our state map, fallback to comment data
				...
				const comment: Comment = {
					...
					replies: [],
					// FIXED: Include like data with proper fallbacks
					likes: likes,
					isLiked: isLiked,
				};
	...
	// FIXED: Load comments from API with like data
	const loadCommentsFromApi = useCallback(async () => {
		if (!postId) return;

		console.log("Loading comments with like data from API for post:", postId);
		setIsLoading(true);
		setError(null);

		try {
			// FIXED: Use the enhanced endpoint that includes like data
			const response = await fetch(
				`/api/comments/posts/${postId}/comments?include_replies=true&include_likes=true`,
				{
					method: "GET",
					headers: {
			...
			console.log("API returned comments with like data:", apiComments);

			// FIXED: Transform API comments to include like data
			const transformedComments: PostComment[] = apiComments.map(
				(apiComment: any) => ({
					id: apiComment.id,
					userId: apiComment.userId,
					postId: apiComment.postId,
					comment: apiComment.comment,
					username: apiComment.commenter?.username || "Unknown User",
	...
	useEffect(() => {
		...
		// Try to use initial comments first, but ALWAYS load fresh data from API
		...
		// Always load fresh data from API to ensure we have the latest like states
		loadCommentsFromApi();
	}, [
		isOpen,
		postId,
	...
	// FIXED: Handle like toggle with proper state management
	...
	const handleRefresh = useCallback(() => {
		...
		setHasInitialized(false);
		setCommentLikeStates(new Map()); // Clear like states to force fresh fetch
		loadCommentsFromApi();
	}, [loadCommentsFromApi]);
							...
							<div className="p-4 space-y-6">
								{processedComments.map((comment) => (
									<CommentThread
										...
										// FIXED: Like handlers with proper state management
