...
const UpdateEvent: React.FC = () => {
	...
	const loaderData = useLoaderData() as UpdateEventLoaderData;

	// Extract the correct data from loader
	const {
		event: eventDetails,
		group: groupDetails,
		user: userData,
	...
	const [imageFile, setImageFile] = useState<File | null>(null); // eslint-disable-line @typescript-eslint/no-unused-vars
	...
	// Check authorization using the correct data structure
	useEffect(() => {
		if (!sessionUser) {
			navigate("/login");
			return;
		...
		// Check if the session user matches the loader user data
		...
		if (sessionUser.id !== groupDetails?.organizerId) {
			navigate("/");
			return;
		}
	}, [sessionUser, userData, groupDetails, navigate]);
	...
	if (!sessionUser || !eventDetails || !groupDetails || !userData) {
		return (
			<div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-slate-50 flex items-center justify-center p-4">
				<div className="bg-white rounded-2xl shadow-lg p-8 max-w-md w-full text-center">
					...
					<p className="text-slate-600 mb-6">Loading event data...</p>
				</div>
			</div>
		);
	}
			...
			<div className="max-w-7xl mx-auto px-4 py-12">
				<div className="grid md:grid-cols-2 gap-8 items-start">
					...
					<div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
						...
						<Form
							method="post"
							action={`/groups/${eventDetails.groupId}/events/${eventDetails.id}/edit`}
