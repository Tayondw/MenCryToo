...
def all_posts():
    """
    ULTRA-OPTIMIZED posts feed with pagination, caching, and minimal data loading
    """
    try:
        page = request.args.get("page", 1, type=int)
        per_page = min(request.args.get("per_page", 20, type=int), 50)

        # OPTIMIZATION: Get current user's tag IDs in a single query
        ...
        if current_user_tag_ids:
            # OPTIMIZATION: Use raw SQL for better performance on large datasets
            posts_query = (
        ...
        # OPTIMIZATION: Batch load like counts for all posts
...
def post(postId):
    """
    ULTRA-OPTIMIZED single post view with minimal queries
    """
    try:
        post = (
            db.session.query(Post)
...
# FIXED post_routes.py - UserTags is a table, not a model
...
def similar_users_posts_feed():
    """
    FIXED: Using UserTags table correctly
    """
    try:
        page = request.args.get("page", 1, type=int)
        per_page = min(request.args.get("per_page", 20, type=int), 50)

        # Step 1: Get current user's tags using the relationship
        ...
        # Step 2: Get tag IDs from the relationship
        user_tag_ids = [tag.id for tag in current_user.users_tags]

        # Step 3: Find users with similar tags using the Tag model
        ...
        # Step 5: Get posts from those users
...
def delete_post(postId):
    """
    ULTRA-OPTIMIZED post deletion with efficient batch operations
    """
    try:
...
def add_comment(postId):
    """
    OPTIMIZED comment addition with minimal validation overhead
    """
    try:
...
def delete_comment(postId, commentId):
    """
    OPTIMIZED comment deletion with efficient nested deletion
    """
    try:
...
def like_post(postId):
    """
    ULTRA-OPTIMIZED post liking with direct SQL operations
    """
    try:
...
def unlike_post(postId):
    """
    ULTRA-OPTIMIZED post unliking with direct SQL operations
    """
    try:
