...
class Comment(db.Model):
    ...
    # FIXED: Add relationship to comment likes
    ...
    def to_dict_with_likes(
        ...
        """
        FIXED: Convert comment to dictionary with like data included and proper like count
        """
        # Get like count and current user like status
        like_count = (
            len(self.comment_likes)
            if hasattr(self, "comment_likes") and self.comment_likes
            else 0
        )

        # Check if current user liked this comment
        is_liked = False
        if current_user_id and hasattr(self, "comment_likes") and self.comment_likes:
            is_liked = any(
        ...
        base_dict = {
            ...
            # FIXED: Always include like data
            "likes": like_count,
            "isLiked": is_liked,
        }
    ...
    def is_liked_by_user(self, user_id):
        ...
        return (
            CommentLike.query.filter_by(comment_id=self.id, user_id=user_id).first()
            is not None
        )

    def toggle_like(self, user_id):
        """Toggle like for a user - returns (is_liked, like_count)"""
        from .comment_like import CommentLike

        existing_like = CommentLike.query.filter_by(
            comment_id=self.id, user_id=user_id
        ).first()
        ...
        if existing_like:
            ...
            new_like = CommentLike(comment_id=self.id, user_id=user_id)
    ...
    @classmethod
    def get_comments_for_post_with_likes(
        cls, post_id, page=1, per_page=20, include_replies=True, current_user_id=None
    ):
        """
        Get comments for a post with like data properly loaded
        """
        from sqlalchemy.orm import joinedload, selectinload

        # Build query with explicit user loading and like data
        query = cls.query.options(
            ...
            # Load like data
            selectinload(cls.comment_likes),
